...
pub fn skeletonize(path: &Path, source: &str) -> SkeletonOutput {
    ...
    let language: tree_sitter::Language = if use_tsx {
...
fn render_top_level(node: Node<'_>, source: &str, indent: usize, prefix: &str) -> Option<String> {
    match node.kind() {
        ...
        "function_signature" => Some(render_function_signature(node, source, indent, prefix)),
        "class_declaration" | "abstract_class_declaration" => {
            Some(render_class(node, source, indent, prefix))
        }
        "lexical_declaration" | "variable_declaration" => {
            render_lexical(node, source, indent, prefix)
        }
        "interface_declaration" => Some(render_interface(node, source, indent, prefix)),
        "type_alias_declaration" => Some(render_type_alias(node, source, indent, prefix)),
        "enum_declaration" => Some(render_enum(node, source, indent, prefix)),
        "ambient_declaration" => render_ambient(node, source, indent),
...
fn render_ambient(node: Node<'_>, source: &str, indent: usize) -> Option<String> {
    let mut cursor = node.walk();
    for child in node.named_children(&mut cursor) {
        if let Some(rendered) = render_top_level(child, source, indent, "declare ") {
            return Some(rendered);
        }
    }
    let literal = slice(source, node.start_byte(), node.end_byte()).trim();
...
fn render_function_signature(node: Node<'_>, source: &str, indent: usize, prefix: &str) -> String {
    let literal = slice(source, node.start_byte(), node.end_byte())
        .trim()
        .trim_end_matches(';')
        .trim_end()
    ...
    for (idx, member) in members.iter().enumerate() {
        let rendered = match member.kind() {
            ...
            "method_signature" | "abstract_method_signature" => {
                Some(render_signature_line(*member, source, indent))
            }
            "public_field_definition" | "field_definition" => {
                Some(render_field(*member, source, indent))
            }
            _ => None,
        };
...
fn render_method(node: Node<'_>, source: &str, indent: usize) -> String {
    ...
    if body.is_some() {
        ...
        let literal = slice(source, node.start_byte(), node.end_byte())
            ...
            .trim_end()
            .to_string();
        push_line(&mut rendered, indent, &format!("{literal};"));
    }
    rendered
}

fn render_signature_line(node: Node<'_>, source: &str, indent: usize) -> String {
    let literal = slice(source, node.start_byte(), node.end_byte())
        .trim()
        .trim_end_matches(';')
        .trim_end()
...
fn render_interface(node: Node<'_>, source: &str, indent: usize, prefix: &str) -> String {
    let body = node.child_by_field_name("body");
    let header_end = body.map(|b| b.start_byte()).unwrap_or(node.end_byte());
    let header = slice(source, node.start_byte(), header_end)
        .trim_end()
    ...
    push_line(&mut rendered, indent, &format!("{prefix}{header} {{"));
    if let Some(body) = body {
        render_interface_body(body, source, indent + 2, &mut rendered);
    }
    push_line(&mut rendered, indent, "}");
    rendered
}

fn render_interface_body(body: Node<'_>, source: &str, indent: usize, output: &mut String) {
    let mut cursor = body.walk();
    let members: Vec<_> = body.named_children(&mut cursor).collect();
    for (idx, member) in members.iter().enumerate() {
        match member.kind() {
            ...
            | "index_signature" => {
                ...
                let literal = slice(source, member.start_byte(), member.end_byte())
                    .trim()
                    .trim_end_matches([';', ','])
                    .trim_end()
                    .to_string();
                push_line(output, indent, &format!("{literal};"));
            }
            _ => {}
        }
    }
...
fn render_type_alias(node: Node<'_>, source: &str, indent: usize, prefix: &str) -> String {
    let value = node.child_by_field_name("value");
    let header_end = value.map(|v| v.start_byte()).unwrap_or(node.end_byte());
    let header = slice(source, node.start_byte(), header_end)
        .trim_end()
...
fn render_enum(node: Node<'_>, source: &str, indent: usize, prefix: &str) -> String {
    let body = node.child_by_field_name("body");
    let header_end = body.map(|b| b.start_byte()).unwrap_or(node.end_byte());
    let header = slice(source, node.start_byte(), header_end)
        .trim_end()
    ...
    if let Some(body) = body {
        ...
        for member in body.named_children(&mut cursor) {
            let literal = slice(source, member.start_byte(), member.end_byte())
                ...
                .to_string();
            if !literal.is_empty() {
                push_line(&mut rendered, indent + 2, &format!("{literal},"));
            }
        }
    }
    push_line(&mut rendered, indent, "}");
...
mod tests {
    ...
    #[test]
    fn emits_interface_and_type_alias() {
        let source = "export type Id = string;\nexport interface User { id: Id; name: string; }\n";
        let output = skeletonize(Path::new("app.ts"), source);
